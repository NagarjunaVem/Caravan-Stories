...
export const register = async (req, res) => {
  ...
  try {
    ...
    const user = await userModel.create({ name, email, password: hashed, role: "citizen" });
...
export const isAuthenticated = async (req, res) => {
  try {
    ...
    if (!userId) return res.status(401).json({ success: false, message: "Not Authorised. Login again" });
...
export const adminCreateUser = async (req, res) => {
  try {
    ...
    if (!name || !email || !password || !role) {
      return res.json({ success: false, message: "name, email, password, and role are required" });
    }
    ...
    return res.json({ success: true, message: "User created", user: { id: user._id, name: user.name, email: user.email, role: user.role } });
  } catch (error) {
    return res.json({ success: false, message: error.message });
  }
};
...
export const updateRole = async (req, res) => {
  try {
    ...
    if (!targetUserId || !role) return res.json({ success: false, message: "targetUserId and role are required" });
    if (!["citizen", "employee", "admin"].includes(role)) {
      return res.json({ success: false, message: "Invalid role" });
    }
    ...
    return res.json({ success: true, message: "Role updated", user: { id: user._id, role: user.role } });
  } catch (error) {
    return res.json({ success: false, message: error.message });
  }
};
...
export const bootstrapAdmin = async (req, res) => {
  try {
    ...
    if (already) return res.json({ success: false, message: "User already exists" });

    const hashed = await bcrypt.hash(password, 10);
    const admin = await userModel.create({ name, email, password: hashed, role: "admin" });

    // Optional: auto-login
    // setAuthCookie(res, admin._id);
